...
def jsontransform(json_in, out):  # transform to form easy to use
    ...
    def Get_specimen_voucher(record):
    ...
    # Get bio_material from given record json
    def Get_bio_material(record):

        obj_list = retrieve_parallel(
            input_dict=record,
            label="GBQualifier_name",
            label_value="bio_material",
            obj="GBQualifier_value",
        )
        ...
        # Get bio_material from given record json
    def Get_isolation_source(record):

        obj_list = retrieve_parallel(
            input_dict=record,
            label="GBQualifier_name",
            label_value="isolation_source",
            obj="GBQualifier_value",
        )
    ...
    def Get_isolation_material(record):

        obj_list = retrieve_parallel(
            input_dict=record,
            label="GBQualifier_name",
            label_value="isolation_material",
            obj="GBQualifier_value",
        )
    ...
    for record in json_data:
        ...
        if (
            ...
            dict_temp["taxonomy"] = record["GBSeq_taxonomy"]
            dict_temp["specimen_name"] = ""
            ...
            dict_temp["organism"] = Get_organism(record)
            dict_temp["voucher"] = Get_voucher(record)
            dict_temp["specimen_voucher"] = Get_specimen_voucher(record)
            ...
            dict_temp["bio_material"]= Get_bio_material(record)
            dict_temp["isolation_source"]= Get_isolation_source(record)
            dict_temp["isolation_material"] = Get_isolation_material(record)
